    ...
    for ihvp_calculator in ihvp_objects:
        for normalize in normalization:
            ...
            for batched_samples in test_set.batch(5):
                ...
                batched_influence_val = influence_calculator._compute_influence_value_from_influence_vector(
        ...
        for normalize in normalization:
            ...
            for batched_samples in test_set.batch(5):
                ...
                for batched_train in train_set.batch(5):
                    batched_influence_val = influence_calculator._compute_individual_influence_values_from_batch(batched_train, batched_samples)
...
def test_compute_influence_values_for_sample_to_evaluate():
    ...
    for ihvp_calculator in ihvp_objects:
        for normalize in normalization:
            ...
            influence_values = []
            for samples_to_evaluate in test_set.batch(5):
                _, samples_inf_values_ds = influence_calculator._compute_influence_values_in_batches(train_set.batch(5), samples_to_evaluate)
                samples_inf_values = []
                for _, inf_values in samples_inf_values_ds:
                    assert inf_values.shape == (5, 5) # (batch_size_evaluate, batch_size_inf_vect_dataset)
                    samples_inf_values.append(inf_values)
    ...
    ds_iterator = iter(test_set.batch(5))
    test_batch = next(ds_iterator)
    temp = influence_calculator._compute_influence_values_in_batches(
                train_set.batch(5),
                test_batch,
                load_influence_vector_ds_path = None,
                save_influence_vector_ds_path = "test_temp/inf_vect"
            )
    assert os.path.exists("test_temp/inf_vect")

    # assert the saved vector dataset is correct
    inf_vect_ds = tf.data.experimental.load("test_temp/inf_vect").batch(5)
    inf_vect = []
    for elt in inf_vect_ds:
        inf_vect.append(elt)
    inf_vect = tf.concat(inf_vect, axis=0)
    ...
    influence_values = []
    for samples_to_evaluate in test_set.batch(5):
        _, samples_inf_values_ds = influence_calculator._compute_influence_values_in_batches(
            train_set.batch(5),
            samples_to_evaluate,
            load_influence_vector_ds_path = "test_temp/inf_vect"
            )
...
def test_compute_influence_values_for_dataset_to_evaluate():
    ...
    for ihvp_calculator in ihvp_objects:
        for normalize in normalization:
            ...
            influence_calculator = FirstOrderInfluenceCalculator(influence_model, train_set.batch(5), ihvp_calculator,
                                                         ...
                                                         shuffle_buffer_size=25,
                                                         normalize=normalize)
            eval_inf_ds = influence_calculator.compute_influence_values_in_batches(
                test_set.batch(5),
                train_set.batch(5)
            )
    ...
    if not os.path.exists("test_temp"):
        os.mkdir("test_temp")
    ds = influence_calculator.compute_influence_values_in_batches(
                test_set.batch(5),
                train_set.batch(5),
    ...
    loaded_inf_vect_ds = influence_calculator.compute_influence_values_in_batches(
        test_set.batch(5),
        train_set.batch(5),
...
def test_top_k():
    ...
    for order in orders:
        for ihvp_calculator in ihvp_objects:
            for normalize in normalization:
                ...
                if order == ORDER.DESCENDING:
                    gt_top_k_influences = tf.math.top_k(gt_inf_values, k=5) # (nb_samples_to_evaluate, 3)
                    gt_top_k_influences_values = gt_top_k_influences.values
                else:
                    gt_top_k_influences = tf.math.top_k(-gt_inf_values, k=5)  # (nb_samples_to_evaluate, 3)
                    gt_top_k_influences_values = - gt_top_k_influences.values
                ...
                for samples_to_evaluate in test_set.batch(5):
                    _, samples_top_k_influences, samples_top_k_samples = influence_calculator._top_k_from_batch(
                        samples_to_evaluate, train_set.batch(5), k=5, order=order
                    )
    ...
    iter_ds = iter(test_set.batch(5))
    batch_test = next(iter_ds)
    if not os.path.exists("test_temp"):
        os.mkdir("test_temp")
    top_k = influence_calculator._top_k_from_batch(
                batch_test,
                train_set.batch(5),
                k=3,
...
def test_top_k_dataset():
        ...
        for ihvp_calculator in ihvp_objects:
            ...
            for normalize in normalization:
                ...
                top_dataset_ds = influence_calculator.top_k(
                    test_set.batch(5), train_set.batch(5), k=3,order=order
                )
    ...
    other_load_ds = influence_calculator.top_k(
                ...
                train_set.batch(5),
                k=3,
                vector_influence_in_cache = False,
                load_influence_vector_ds_path="test_temp/influence_vector_ds",
            )
    top_k_influences, top_k_samples = [], []
...
def test_cnn_shapes():
    ...
    for ihvp_calculator in ihvp_objects:
        ...
        inf_val_from_tensor = influence_calculator._compute_individual_influence_values_from_batch(
        ...
        batch_samples, iter_inf_val_sample_ds = influence_calculator._compute_influence_values_in_batches(
            train_set,
            test_batch
        )
        ...
        inf_val_dataset = influence_calculator.compute_influence_values_in_batches(
            test_set,
            train_set
        )
        ...
        # top_k
        _, top_k_inf_val, top_k_samples = influence_calculator._top_k_from_batch(
            test_batch,
            train_set,
            k=3,
            d_type=tf.float64
        ...
        # top_k_dataset
        top_k_dataset = influence_calculator.top_k(
            test_set,
            train_set,
            k=3,
            d_type=tf.float64
        ...
        influence_group = influence_calculator.compute_influence_group(train_set)
